...
class EventController extends EntityController {
	...
	public function showAction( JobHandler $jobHandler ): Response {
		...
		foreach ( [ 'Participants', 'Categories' ] as $formType ) {
			if ( $formType === 'Categories' ) {
				...
				if ( $this->event->getCategories()->isEmpty() ) {
					...
					$defaultDomain = $this->event->getWikis()->count() === 1 && !$firstWiki->isFamilyWiki()
						? $firstWiki->getDomain()
						: '';
		...
		$isOnlyWikidata = $this->event->getWikis()->count() === 1 && $this->event->getWikiByDomain( 'www.wikidata' );
	 ...
	 * @param Event $event
	 * @param string $type Plural form, matching App\Form\*, e.g. 'Participants' or 'Categories'.
	...
	private function handleFormSubmission(
		Event $event,
		string $type,
		string $redirect = 'Event'
	): RedirectResponse|FormInterface {
		$form = $this->createForm( 'App\Form\\' . $type . 'Type', $event, [
