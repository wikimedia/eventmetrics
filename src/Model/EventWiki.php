 ...
 * @ORM\Entity(repositoryClass="App\Repository\EventWikiRepository")
 * @ORM\Table(
 *     name="event_wiki",
 *     indexes={
...
class EventWiki {
	 ...
	 * @var string|null Domain of the wiki, without the .org.
	 */
	 ...
	 * @param string|null $domain Domain name of the wiki, without the .org.
	 */
	...
	public function getFamilyName(): ?string {
		foreach ( self::FAMILY_NAMES as $family ) {
			if ( str_contains( $this->domain, $family ) ) {
				return $family;
			}
		}
	...
	public function getStatistic( string $metric ): ?EventWikiStat {
		$ewStats = $this->stats->filter( static function ( EventWikiStat $stat ) use ( $metric ) {
	...
	public function getPages(): array {
		return array_unique(
			array_merge( $this->getPagesCreated(), $this->getPagesImproved(), $this->getPagesFiles() )
		);
	}
	...
	protected function setPageIds( string $type, ?array $ids ):void {
		...
		$filteredIds = array_filter( $ids, static function ( $id ) {
			if ( !is_numeric( $id ) ) {
	...
	public function validateWiki( ExecutionContextInterface $context ): void {
		if ( preg_match( self::VALID_WIKI_PATTERN, (string)$this->domain ) !== 1 ) {
