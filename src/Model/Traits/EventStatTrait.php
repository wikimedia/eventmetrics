 ...
 * @property Collection<EventStat> $stats
 * @property Collection<EventWiki> $wikis
...
trait EventStatTrait {
	...
	public function getStatistic( string $metric ): ?EventStat {
		/** @var Collection<EventStat> $ewStats */
		$ewStats = $this->stats->filter( static function ( EventStat $stat ) use ( $metric ) {
	...
	public function getAvailableMetrics( ?string $family = null ): array {
		...
		foreach ( $this->wikis as $wiki ) {
			$isFamily = $family === null || $wiki->getFamilyName() === $family;
		...
		return array_filter( self::AVAILABLE_METRICS, static function ( $offset, $metric ) use ( $metricKeys ): bool {
