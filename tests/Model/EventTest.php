...
class EventTest extends EventMetricsTestCase {
	...
	public function testAddRemoveStatistics(): void {
		...
		static::assertCount( 0, $event->getStatistics() );
		...
		$event->addStatistic( $eventStat );
		static::assertCount( 1, $event->getStatistics() );
	...
	public function testAddRemoveParticipant(): void {
		...
		static::assertCount( 0, $event->getParticipants() );
		...
		// Try adding the same one, which shouldn't duplicate.
		$event->addParticipant( $participant );
		static::assertSame( 1, $event->getNumParticipants() );

		// Removing the participant.
		$event->removeParticipant( $participant );
		static::assertSame( 0, $event->getNumParticipants() );
	...
	public function testAddRemoveWiki(): void {
		...
		// Try adding the same one, which shouldn't duplicate.
		$event->addWiki( $wiki );
		static::assertCount( 1, $event->getWikis() );

		// Removing the wiki.
		$event->removeWiki( $wiki );
		static::assertCount( 0, $event->getWikis() );
	...
	public function testJobs(): void {
		...
		static::assertSame( 0, $event->getNumJobs() );
	...
	public function testWikiFamilies(): void {
		...
		static::assertSame( 0, $event->getChildWikis()->count() );
	...
	/**
	 * @covers \App\Model\Event::getWikisByFamily()
	 */
	...
	/**
	 * @covers \App\Model\Event::getWikisWithoutCategories()
	 */
