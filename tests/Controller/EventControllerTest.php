 ...
 * @covers \App\Controller\EventController
 * @group replicas
 */
class EventControllerTest extends DatabaseAwareWebTestCase {
	/** @var int|null ID of the Program, used for routing. */
	private ?int $programId;

	/** @var ?int|null ID of the Event, used for routing. */
	private ?int $eventId;

	private ?EventRepository $eventRepo;
	private ?EventWikiRepository $eventWikiRepo;
	...
	public function setUp(): void {
		...
		$this->eventWikiRepo = static::getContainer()->get( EventWikiRepository::class );
	...
	public function testWorkflow(): void {
		...
		/** @var Program $program */
		$program = $this->programRepo->findOneBy( [ 'title' => 'My_fun_program' ] );
	...
	private function createSpec(): void {
		...
		/** @var Event $event */
		$event = $this->eventRepo->findOneBy( [ 'title' => 'The_Lion_King' ] );
		...
		$eventWiki = $this->eventWikiRepo->findOneBy( [ 'event' => $event ] );
	...
	private function updateSpec(): void {
		...
		$eventWiki = $this->eventWikiRepo->findOneBy( [ 'domain' => 'de.wikipedia' ] );
		static::assertNull( $eventWiki );

		$event = $this->eventRepo->findOneBy( [ 'title' => 'Pinocchio' ] );
	...
	public function familyWikiSpec(): void {
		...
		$eventWiki = $this->eventWikiRepo->findOneBy( [ 'domain' => 'fr.wikipedia' ] );
		static::assertNotNull( $eventWiki );
		...
		$eventWiki = $this->eventWikiRepo->findOneBy( [ 'domain' => 'en.wikipedia' ] );
		static::assertNull( $eventWiki );
		$eventWiki = $this->eventWikiRepo->findOneBy( [ 'domain' => 'fr.wikipedia' ] );
		static::assertNull( $eventWiki );
		$eventWikis = $this->eventWikiRepo->findBy( [ 'event' => $event ] );

		// *this-.wikipedia and commons.wikimedia should still remain.
		$domains = array_map( static function ( EventWiki $eventWiki ) {
	...
	public function categoriesSpec(): void {
		...
		static::assertSame( '', $form['categoryForm[categories][0][title]']->getValue() );
		...
		static::assertStringContainsString(
			'has-error',
			$this->crawler->filter( '#categoryForm_categories_0_title' )
				->ancestors()->attr( 'class' )
		);
		...
		$event = $this->eventRepo->findOneBy( [ 'title' => 'Pinocchio' ] );
		// Clear cache, unclear why this is needed.
		$this->entityManager->refresh( $event );
	...
	private function cloneSpec(): void {
		...
		$event = $this->eventRepo->findOneBy( [ 'title' => 'Pinocchio_II' ] );
	...
	private function deleteSpec(): void {
		static::assertCount(
			// There was a cloned event, see self::cloneSpec()
			2,
